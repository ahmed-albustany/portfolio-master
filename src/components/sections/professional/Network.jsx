...
import {
  HiGlobe, HiChevronDown, HiChevronUp,
  HiCheckCircle, HiStatusOnline,
} from 'react-icons/hi';
import { useFirestore } from '@/hooks/useFirestore';
import { getNetwork } from '@/firebase/firestore';
import { fallbackNetwork } from '@/data/fallbackData';
...
function getTypeColor(type) {
  return typeColors[(type || '').toLowerCase()] || DEPT_COLOR;
}
...
function NetworkCard({ item }) {
  ...
  const typeColor = getTypeColor(item.networkType);
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            ...
            >
              <HiGlobe className="w-5 h-5" style={{ color: DEPT_COLOR }} />
            </div>
            <div>
              <h3 className="text-sm sm:text-base font-heading font-bold"
                  style={{ color: 'var(--color-text-primary)' }}>
                {item.title || item.infrastructure || 'Network Infrastructure'}
              </h3>
              {item.organization && (
        ...
        {protocols.length > 0 && (
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {protocols.map((p) => (
                <span key={p} className="px-1.5 py-0.5 text-[10px] font-mono rounded"
...
export default function Network() {
  const { data: items, loading } = useFirestore(getNetwork, fallbackNetwork);
  const [ref, inView] = useInView({ threshold: 0.1, triggerOnce: true });

  if (loading) {
    return (
      <section id="network" className="section-padding" style={{ backgroundColor: 'var(--color-bg-secondary)' }}>
  ...
  return (
    <section id="network" className="section-padding relative overflow-hidden"
        ...
        <motion.div ref={ref} variants={container} initial="hidden" animate={inView ? 'visible' : 'hidden'}>
          <motion.div variants={fadeUp} className="text-center mb-10 sm:mb-12">
            ...
            <h2 className="heading-secondary mb-3" style={{ color: 'var(--color-text-primary)' }}>
              Network <span style={{ color: DEPT_COLOR }}>Administration</span>
            </h2>
          </motion.div>

          {(!items || items.length === 0) ? (
            <EmptyState icon={HiGlobe} title="No Network Data"
                        description="No network administration data yet — add from admin panel." color={DEPT_COLOR} />
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              {items.map((item) => <NetworkCard key={item.id} item={item} />)}
            </div>
          )}
        </motion.div>
      </div>
