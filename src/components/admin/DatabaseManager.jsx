...
import {
  ...
  HiExclamationCircle, HiCheckCircle, HiDatabase,
} from 'react-icons/hi';
import { getDatabase, addDatabase, updateDatabase, deleteDatabase } from '@/firebase/firestore';
...
function ItemForm({ item, onSave, onCancel }) {
  ...
  const [queryTypes, setQueryTypes] = useState(item?.queryTypes || []);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        dbSystem: data.dbSystem.trim(), scale: data.scale.trim(),
        duration: data.duration.trim(), description: data.description.trim(),
        queryTypes, tools, tasks, achievements,
      };
      if (isEditing) await updateDatabase(item.id, payload);
      else await addDatabase(payload);
      onSave();
    ...
    <div className="rounded-xl p-5 sm:p-6" style={{ backgroundColor: '#0D1520', border: '1px solid #1A2840' }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <AdminField label="DB System" placeholder="e.g. MySQL, MongoDB" {...register('dbSystem')} />
          <AdminField label="Scale" placeholder="e.g. 50GB, 1M+ rows" {...register('scale')} />
        ...
        <div>
          <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Query Types</label>
          <TagInput value={queryTypes} onChange={setQueryTypes} accent={ACCENT} placeholder="e.g. CRUD, Joins..." />
        </div>
        <div>
          <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Tools</label>
          <TagInput value={tools} onChange={setTools} accent="#0066FF" placeholder="e.g. phpMyAdmin..." />
        </div>
        <div>
          <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Tasks</label>
          <TagInput value={tasks} onChange={setTasks} accent="#00D4FF" placeholder="Add task..." />
        </div>
        <div>
...
export default function DatabaseManager() {
  ...
  const fetchItems = async () => { setLoading(true); try { setItems(await getDatabase()); } catch { showToast('Failed to load', 'error'); } finally { setLoading(false); } };
  useEffect(() => { fetchItems(); }, []);

  const handleDelete = async (item) => {
    if (!window.confirm(`Delete "${item.title}"?`)) return;
    try { await deleteDatabase(item.id); setItems((p) => p.filter((i) => i.id !== item.id)); showToast('Deleted'); }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-heading font-bold text-white">Database</h2>
      ...
      {!loading && items.length === 0 && !formMode && (
        <div className="text-center py-16"><HiDatabase className="w-12 h-12 mx-auto mb-3 text-[#1A2840]" /><p className="text-xs font-mono text-[#334155]">No entries</p></div>
      )}
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl" style={{ backgroundColor: '#0D1520', border: '1px solid #1A2840' }}>
              <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${ACCENT}15` }}>
                <HiDatabase className="w-4 h-4" style={{ color: ACCENT }} />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-white truncate">{item.title}</h4>
                <p className="text-[11px] font-mono text-[#64748B] truncate">{item.dbSystem} {item.organization && `\u2022 ${item.organization}`}</p>
              </div>
              {item.scale && <span className="hidden sm:block px-2 py-1 text-[9px] font-mono font-bold uppercase rounded" style={{ color: ACCENT, backgroundColor: `${ACCENT}10` }}>{item.scale}</span>}
