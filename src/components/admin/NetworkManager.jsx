...
import {
  ...
  HiExclamationCircle, HiCheckCircle, HiGlobe,
} from 'react-icons/hi';
import { getNetwork, addNetwork, updateNetwork, deleteNetwork } from '@/firebase/firestore';

const ACCENT = '#00FF88';
const NET_TYPES = ['LAN', 'WAN', 'WiFi', 'VPN', 'VLAN', 'SD-WAN'];
...
function ItemForm({ item, onSave, onCancel }) {
  ...
  const [protocols, setProtocols] = useState(item?.protocols || []);
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    defaultValues: {
      ...
      networkType: item?.networkType || 'LAN',
      scale: item?.scale || '',
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        networkType: data.networkType, scale: data.scale.trim(),
        duration: data.duration.trim(), description: data.description.trim(),
        protocols, equipment, responsibilities,
      };
      if (isEditing) await updateNetwork(item.id, payload);
      else await addNetwork(payload);
      onSave();
    ...
    <div className="rounded-xl p-5 sm:p-6" style={{ backgroundColor: '#0D1520', border: '1px solid #1A2840' }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Network Type</label>
            <select className="admin-input" {...register('networkType')}>
              {NET_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <AdminField label="Scale" placeholder="e.g. 500+ nodes" {...register('scale')} />
        ...
        <div>
          <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Protocols</label>
          <TagInput value={protocols} onChange={setProtocols} accent={ACCENT} placeholder="e.g. TCP/IP, OSPF..." />
        </div>
        <div>
          <label className="block text-[10px] font-mono font-semibold text-[#64748B] uppercase tracking-widest mb-2">Equipment</label>
          <TagInput value={equipment} onChange={setEquipment} accent="#0066FF" placeholder="e.g. Cisco 2960..." />
        </div>
        <div>
          ...
          <TagInput value={responsibilities} onChange={setResponsibilities} accent="#00D4FF" placeholder="Add responsibility..." />
        </div>
        <div className="flex items-center gap-3 pt-2">
          <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 px-5 py-2.5 text-[11px] font-mono font-bold uppercase tracking-widest rounded-lg disabled:opacity-50 hover:brightness-110" style={{ backgroundColor: ACCENT, color: '#060B14' }}>
...
export default function NetworkManager() {
  ...
  const fetchItems = async () => { setLoading(true); try { setItems(await getNetwork()); } catch { showToast('Failed to load', 'error'); } finally { setLoading(false); } };
  useEffect(() => { fetchItems(); }, []);

  const handleDelete = async (item) => {
    if (!window.confirm(`Delete "${item.title}"?`)) return;
    try { await deleteNetwork(item.id); setItems((p) => p.filter((i) => i.id !== item.id)); showToast('Deleted'); }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-heading font-bold text-white">Network</h2>
      ...
      {!loading && items.length === 0 && !formMode && (
        <div className="text-center py-16"><HiGlobe className="w-12 h-12 mx-auto mb-3 text-[#1A2840]" /><p className="text-xs font-mono text-[#334155]">No entries</p></div>
      )}
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl" style={{ backgroundColor: '#0D1520', border: '1px solid #1A2840' }}>
              <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${ACCENT}15` }}>
                <HiGlobe className="w-4 h-4" style={{ color: ACCENT }} />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-white truncate">{item.title}</h4>
                <p className="text-[11px] font-mono text-[#64748B] truncate">{item.organization}</p>
              </div>
              {item.networkType && <span className="hidden sm:block px-2 py-1 text-[9px] font-mono font-bold uppercase rounded" style={{ color: ACCENT, backgroundColor: `${ACCENT}10` }}>{item.networkType}</span>}
